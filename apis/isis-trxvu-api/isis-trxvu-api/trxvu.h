...
typedef enum
{
    RADIO_IDLE_UNKNOWN = 0, /**< Dummy value to indicate no change should be
                               made */
...
typedef enum
{
    RADIO_TX_TELEM_ALL, /**< Returns the current measurements of all the
                           transmitter's telemetry channels */
    RADIO_TX_TELEM_LAST, /**< Returns the telemetry channels that were sampled
                            during the last frame transmission */
    RADIO_TX_UPTIME, /**< Returns the amount of time, in seconds, that the
                        transmitter portion of the radio has been active */
    RADIO_TX_STATE, /**< Returns the current state of the transmitter */
    RADIO_RX_TELEM_ALL, /**< Returns the current measurements of all the
                           receiver's telemetry channels */
    RADIO_RX_UPTIME /**< Returns the amount of time, in seconds, that the
                       receiver portion of the radio has been active */
} RadioTelemType;
...
typedef struct
{
    ...
    uint16_t max_frames; /**< Maximum number of frames that can be in the
                            buffer */
} trx_prop;

/**
 * Transmitter raw telemetry fields returned from ::RADIO_TX_TELEM_ALL and
 * ::RADIO_TX_TELEM_LAST requests
 */
typedef struct
{
    uint16_t inst_RF_reflected; /**< Instantaneous RF reflected power at
                                   transmitter port */
    uint16_t inst_RF_forward; /**< Instantaneous RF forward power at
                                 transmitter port */
...
/**
 * Receiver raw telemetry fields returned from ::RADIO_RX_TELEM_ALL telemetry
 * request
 */
typedef struct
{
    uint16_t inst_doppler_offset; /**< Instantaneous Doppler offset of signal
                                     at receiver port */
    uint16_t supply_current; /**< Total supply current */
    uint16_t supply_voltage; /**< Power bus voltage */
    uint16_t temp_oscillator; /**< Local oscillator temperature */
    uint16_t temp_power_amp; /**< Power amplifier temperature */
    uint16_t inst_signal_strength; /**< Instantaneous signal strength of the
...
typedef union
{
    ...
    trxvu_uptime uptime; /**< Returned by ::RADIO_TX_UPTIME and
                            ::RADIO_RX_UPTIME */
    trxvu_tx_telem_raw tx_telem; /**< Returned by ::RADIO_TX_TELEM_ALL and
                                    ::RADIO_TX_TELEM_LAST */
...
typedef struct
{
    uint16_t interval; /**< Interval (in seconds) at which the beacon message
                          should be sent */
...
typedef struct
{
    ...
    uint16_t doppler_offset; /**< ADC value of doppler shift at receive time
                                (convert with ::get_doppler_offset)*/
    uint16_t signal_strength; /**< ADC value of signal strength at receive
                                 time (convert with ::get_signal_strength)*/
} radio_rx_header;
...
/**
 * Start a thread to kick the radio's watchdogs at an interval of (timeout/3)
 * seconds
 * @return KRadioStatus `RADIO_OK` if OK, error otherwise
 */
...
/**
 * Send a message to the transmit buffer, but use the specified call-signs
...
/**
 * Set the automatic periodic beacon, but use the specified call-signs instead
 * of the defaults
 * @param [in] to AX.25 call-sign for message sender
 * @param [in] from AX.25 call-sign for message destination
 ...
 * @return KRadioStatus RADIO_OK if OK, error otherwise
 */
KRadioStatus k_radio_init(char * bus, trx_prop tx, trx_prop rx,
                          uint16_t timeout);
/**
...
/**
 * Configure the radio
 * @note This function might not be implemented for all radios. See specific
 * radio API documentation for configuration availability and structure
 ...
 * @return KRadioStatus RADIO_OK if a message was received successfully,
 * RADIO_RX_EMPTY if there are no messages to receive, error otherwise
 */
 ...
 *
 * @param rate Data rate. Should be `RADIO_TX_RATE_1200`,
 * `RADIO_TX_RATE_2400`, `RADIO_TX_RATE_4800`, or `RADIO_TX_RATE_9600`
 * @return KRadioStatus `RADIO_OK` on success, otherwise error
 */
 ...
 *
 * @param[out] buffer Pointer to storage area. Should point to a
 * ::trxvu_tx_telem_raw structure if ::RADIO_TX_TELEM_ALL or
 * ::RADIO_TX_TELEM_LAST is being requested
 * @param[in] type Telemetry type to fetch. Should be ::RADIO_TX_TELEM_ALL,
 * ::RADIO_TX_TELEM_LAST, ::RADIO_TX_UPTIME, or ::RADIO_TX_STATE
 * @return KRadioStatus `RADIO_OK` on success, otherwise error
 */
 ...
 * @param type Type of reset to be performed. Should be `RADIO_RESET_SOFT` or
 * `RADIO_RESET_HARD`
 * @return KRadioStatus `RADIO_OK` if OK, error otherwise
 */
 ...
 *
 * @param[out] buffer Pointer to storage area. Should point to a
 * ::trxvu_rx_telem_raw structure if ::RADIO_RX_TELEM_ALL is being requested
 * @param[in] type Telemetry type to fetch. Should be ::RADIO_RX_TELEM_ALL or
 * ::RADIO_RX_UPTIME
 * @return KRadioStatus `RADIO_OK` on success, otherwise error
 */
