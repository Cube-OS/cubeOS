...
typedef struct
{
    ...
    uint8_t config; /**< Parameter updated since system startup? 0 - No, 1 -
                       Yes */
...
{
    imtq_resp_header hdr; /**< Response message header */
    imtq_mtm_data data; /**< MTM measurement data. Units dependent on function
                           used */
    uint8_t act_status; /**< Coils actuation status during measurement. 0 -
                           Not actuating, 1 - Actuating */
...
typedef imtq_axis_msg imtq_coil_current; /**< Coil currents in
                                            [10<sup>-4</sup> A] returned by
                                            ::k_imtq_get_coil_current */
typedef imtq_axis_msg imtq_coil_temp; /**< Coil temperatures in
                                         [<sup>o</sup>C] returned by
                                         ::k_imtq_get_coil_temps */
typedef imtq_axis_msg imtq_dipole; /**< Commanded actuation dipole in
                                      [10<sup>-4</sup> Am<sup>2</sup>]
                                      returned by ::k_imtq_get_dipole */
...
typedef struct
{
    ...
    imtq_mtm_data    mtm_raw; /**< Raw MTM data in [7.5*10<sup>-9</sup> T] per
                                 count */
...
typedef struct
{
    ...
    imtq_test_result step; /**< Measurements during actuation of requested
                              axis */
...
{
    imtq_test_result init; /**< Measurements before actuation */
    imtq_test_result x_pos; /**< Measurements during actuation of positive
                               x-axis */
    imtq_test_result x_neg; /**< Measurements during actuation of negative
                               x-axis */
    imtq_test_result y_pos; /**< Measurements during actuation of positive
                               y-axis */
    imtq_test_result y_neg; /**< Measurements during actuation of negative
                               y-axis */
    imtq_test_result z_pos; /**< Measurements during actuation of positive
                               z-axis */
    imtq_test_result z_neg; /**< Measurements during actuation of negative
                               z-axis */
    imtq_test_result final; /**< Measurements after actuation */
...
typedef struct
{
    ...
    imtq_axis_data dipole; /**< Commanded actuation dipole in [10<sup>-4</sup>
                              Am<sup>2</sup>] */
 ...
 * @param [out] data Pointer to storage for returned system uptime. If the
 * value is zero, then the ADCS is offline.
 ...
 * @param [out] mode Pointer to storage which the mode value should be copied
 * to
 * @return KADCSStatus ADCS_OK if OK, error otherwise
 */
 ...
 *
 * Measurement units are in [7.5 * 10<sup>-9</sup> T], as documented in the <a
 * href="http://www.compotrade.ru/Components/Sensixs/XEN1210.pdf">XEN1210
 * datasheet</a>
...
/**
 * Get the current system configuration and last self-test results and add
 * them to the telemetry JSON
 * @param [out] buffer Pointer to telemetry JSON structure
 * @return KADCSStatus `ADCS_OK` if OK, error otherwise
 */
