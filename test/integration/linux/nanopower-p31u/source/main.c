...
KEPSStatus get_heater()
{
    ...
    if (status != EPS_OK)
    {
        fprintf(fp, "[Get Heater Test] Failed to get EPS heaters status: %d\n",
                status);
        fprintf(stderr,
                "[Get Heater Test] Failed to get EPS heaters status: %d\n",
                status);
        return EPS_ERROR;
    }
...
KEPSStatus get_config()
{
    ...
    {
        fprintf(
            fp, "[Get Config Test] Error/s occurred while getting EPS system config: %d\n",
            status);
        fprintf(
            stderr, "[Get Config Test] Error/s occurred while getting EPS system config: %d\n",
            status);
        return EPS_ERROR;
    }
    ...
    fprintf(fp, "\nVBoost - %d %d %d\n", sys_config.vboost[0],
            sys_config.vboost[1], sys_config.vboost[2]);
    ...
    {
        fprintf(
            fp, "[Get Config Test] Error/s occurred while getting EPS battery config: %d\n",
            status);
        fprintf(
            stderr, "[Get Config Test] Error/s occurred while getting EPS battery config: %d\n",
            status);
        return EPS_ERROR;
    }
...
KEPSStatus get_housekeeping()
{
    ...
    {
        fprintf(
            fp, "[Housekeeping Test] Error/s occurred while getting EPS deploy status: %d\n",
            status);
        fprintf(
            stderr, "[Housekeeping Test] Error/s occurred while getting EPS deploy status: %d\n",
            status);
        return EPS_ERROR;
    }

    fprintf(fp, "\nHousekeeping Data (HK2):\n");
    fprintf(fp, "VBoost - %d %d %d\n", hk.vboost[0], hk.vboost[1],
            hk.vboost[2]);
    fprintf(fp, "Battery voltage - %dmV\n", hk.vbatt);
    fprintf(fp, "Input currents - %d %d %d\n", hk.curin[0], hk.curin[1],
            hk.curin[2]);
...
KEPSStatus system_config()
{
    ...
    if (status != EPS_OK)
    {
        fprintf(
            stderr,
            "[System Config Test] Failed to reset EPS system config: %d\n",
            status);
        fprintf(
            stderr,
            "[System Config Test] Failed to reset EPS system config: %d\n",
    ...
    {
        fprintf(
            fp, "[System Config Test] Error/s occurred while getting EPS system config: %d\n",
            status);
        fprintf(
            stderr, "[System Config Test] Error/s occurred while getting EPS system config: %d\n",
            status);
        return EPS_ERROR;
    }
    ...
    {
        fprintf(
            fp, "[System Config Test] Current system config doesn't match desired config: %d\n",
            status);
        fprintf(
            stderr, "[System Config Test] Current system config doesn't match desired config: %d\n",
            status);
        return EPS_ERROR;
    }
...
KEPSStatus battery_config()
{
    ...
    if (status != EPS_OK)
    {
        fprintf(
            fp,
            "[Battery Config Test] Failed to reset EPS battery config: %d\n",
            status);
        fprintf(
            stderr,
            "[Battery Config Test] Failed to reset EPS battery config: %d\n",
    ...
    {
        fprintf(
            fp, "[Battery Config Test] Failed to save EPS configuration: %d\n",
            status);
        fprintf(
            stderr,
            "[Battery Config Test] Failed to save EPS configuration: %d\n",
            status);
        return EPS_ERROR;
    }
    ...
    {
        fprintf(
            fp, "[Battery Config Test] Error/s occurred while getting EPS battery config: %d\n",
            status);
        fprintf(
            stderr, "[Battery Config Test] Error/s occurred while getting EPS battery config: %d\n",
            status);
        return EPS_ERROR;
    }
    ...
    {
        fprintf(
            fp, "[Battery Config Test] Current battery config doesn't match desired config: %d\n",
            status);
        fprintf(
            stderr, "[Battery Config Test] Current battery config doesn't match desired config: %d\n",
            status);
        return EPS_ERROR;
    }
...
KEPSStatus set_output()
{
    ...
    const struct timespec DELAY = { .tv_sec = 0, .tv_nsec = 500000000 };
...
KEPSStatus single_output()
{
    ...
    if (status != EPS_OK)
    {
        fprintf(
            fp,
            "[Set Single Output Test] Failed to set single EPS output: %d\n",
            status);
        fprintf(
            stderr,
            "[Set Single Output Test] Failed to set single EPS output: %d\n",
...
KEPSStatus reset_counters()
{
    ...
    if (status != EPS_OK)
    {
        fprintf(fp, "[Reset Counters Test] Failed to reset EPS counters: %d\n",
                status);
        fprintf(stderr,
                "[Reset Counters Test] Failed to reset EPS counters: %d\n",
                status);
        return EPS_ERROR;
    }
    ...
    if (new_count >= current_count)
    {
        ...
        fprintf(stderr,
                "[Reset Counters Test] Failed to reset EPS counters\n");
        return EPS_ERROR;
    }
...
KEPSStatus passthrough()
{
    ...
    if (status != EPS_OK)
    {
        fprintf(
            fp,
            "[Passthrough Test] Failed to send EPS passthrough packet: %d\n",
            status);
        fprintf(
            stderr,
            "[Passthrough Test] Failed to send EPS passthrough packet: %d\n",
...
int main(int argc, char * argv[])
{
    ...
    {
        fprintf(stderr, "One or more NanoPower tests have failed. See "
                        "nanopower-results.txt for info\n");
