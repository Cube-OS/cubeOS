...
///
/// ```
/// use cubeos_service::process_errors;
...
mod tests {
    ...
    fn run_push() {
        ...
        assert_eq!(result, Err("TopError: top, RootError: root".to_owned()));
        assert_eq!(
            vec!["test_func (services/cubeos-service/src/macros.rs:305): TopError: top, RootError: root".to_owned()],
            *master_err.read().unwrap()
        );
    }
