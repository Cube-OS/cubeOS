CubeOS Linux Overview
====================

Introduction
------------

This is intended as a higher-level overview of the CubeOS Linux
configuration, installation, and distribution for the CubeOS clients'
embedded systems.

The high level components of every system will be:

- Low-level bootloader/s
- U-Boot (mid-level bootloader. Loads the CubeOS Linux kernel)
- CubeOS
...
`CubeOS U-Boot Repo <https://github.com/cubeos/uboot>`__
...
If you have a CubeOS Vagrant image currently running, the FTDI connection will
be automatically passed through. You can use the included minicom configuration
to quickly connect to the board via the ``minicom cubeos`` command in the VM's
command console.

.. note:: 

    If a CubeOS Vagrant image is running, you will be unable to establish a serial
...
By default, there are two user accounts available: "root" (the superuser), and "cubeos" (a normal user).
Both have a default password of "CubeOS123". For more information, see the :ref:`user-accounts` section.
...
All built-in CubeOS services will reside in the /usr/sbin directory. This
...
upgrade the CubeOS Linux kernel and root file system.

Users
~~~~~

By default, there are only two users defined to the CubeOS Linux system:
'root' and 'cubeos'. To add more users, the Linux ``adduser`` or
...
If you already have CubeOS Linux installed on your system, but would like to
...
Should your CubeOS Linux kernel become corrupted (as indicated by failing to
...
1. Reload the current version of CubeOS Linux from the kpack\*.itb file
   in the upgrade partition
2. Reload the previous version of CubeOS Linux from the kpack\*.itb file
   in the upgrade partition
3. Reload the base version of CubeOS Linux from the kpack-base.itb file
   in the upgrade partition
...
:doc:`CubeOS Linux Recovery doc <cubeos-linux-recovery>`
