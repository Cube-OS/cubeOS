CubeOS Coding and Documentation Standards
========================================

This is a doc to maintain the current naming and coding standards when
working with the CubeOS products.

Product Names
#############

The general naming scheme for products is "CubeOS {component}". Note that there is a space separating the two words.
...
CubeOS (note the capitalization) refers to the entire system, not a specific component.

Component Names
###############

Components within CubeOS breakdown as follows:
...
Components added to the system should be referred to/labeled accordingly. See the :doc:`CubeOS Ecosystem <../ecosystem/index>` document for more information on each component's roll.
...
For a doc like "Installing the CubeOS SDK", we can assume that the reader
...
In contrast, with a doc like "Developing CubeOS Modules" we can expect
...
.. note::
    ...
    CubeOS is currently using the ``1.0.0-stable`` version of ``rustfmt``
...
`CubeOS
repo <https://github.com/cubeos/cubeos/blob/master/.clang-format>`__
...
**Do not use tabs. Instead, use 4 spaces.** CubeOS developers and
...
Some of the CubeOS code uses or extends external projects. In this case
where you are adding a new file, use the CubeOS standards. If you are
